...
def discriminator(image, options, reuse=False, name="discriminator"):
    ...
    with tf.variable_scope(name):
        ...
        h4 = conv3d(h3, 1, s=(1,1,1), name='d_h3_pred')
        # h4 is (32 x 32 x 1)
        return h4
...
def generator_resnet(image, options, reuse=False, name="generator"):
    with tf.variable_scope(name):
        ...
        def residule_block(x, dim, ks=4, s=2, name='res'):
            # p = int((ks - 1) / 2)
        ...
        c1 = tf.nn.relu(instance_norm(conv3d(image, options.gf_dim, 7, 1, padding='SAME', name='g_e1_c'), 'g_e1_bn'))
